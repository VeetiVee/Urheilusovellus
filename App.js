...
function NewTraining() {
  ...
  useEffect(() => {
    db.transaction(tx => {
      tx.executeSql('create table if not exists pastTraining (id integer primary key not null, move text, weight text);');
    }, null, updateList); 
  }, []);

  const saveTraining = () => {
    db.transaction(tx => {
        tx.executeSql('insert into pastTraining (move, weight) values (?, ?);', [move, weight]);    
      }, null, updateList
    )
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList 
        ...
        renderItem={({item}) => <View style={styles.listcontainer}><Text style={{fontSize: 18}}>{item.move}, {item.weight}</Text>
